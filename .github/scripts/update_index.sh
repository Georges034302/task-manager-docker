...
# Robustly extract tasks (handling missing "Done Tasks:")
TODO_TASKS=$(grep -A 1000 "ToDo Tasks:" "$1" | sed '1d;/Done Tasks:/q')
DONE_TASKS=$(grep "Done Tasks:" "$1" && grep -A 1000 "Done Tasks:" "$1" | sed '1d' || true) # Handles missing marker
...
# Update all pre blocks (correct variable usage)
