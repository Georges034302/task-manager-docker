...
# Function to update pre blocks (using sed - FINALLY AND DEFINITIVELY CORRECT AND PORTABLE)
update_pre() {
  local pre_id="$1"
  local content="$2"
  local html_file="$3"
  ...
  # Use sed to replace the content within the <pre> block (using ||| as delimiter and portable regex)
  sed -i "/<pre id=\"$pre_id\">/,/<\/pre>/ {
    /<pre id=\"$pre_id\">/ {
